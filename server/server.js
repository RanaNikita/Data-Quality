...
app.get('/api/agents/:agentName', async (req, res) => {
  try {
    ...
    // 🔒 Only allow agents from the centralized allow-list
    const allowedSet = new Set(AGENT_ALLOW_LIST);
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 400) {
        ...
        errorParts.push(
          '💡 Tip: Check your SNOWFLAKE_HOST in the backend .env file. Make sure it\'s correct and accessible.'
        );
      }
      console.error('❌ Snowflake Agent API error:', response.status, errorParts.join('\n'));
      return res.status(response.status).json({ errorParts }); // Send as array instead of single string
